...
internal static partial class LogExtensions
{
    [LoggerMessage(
        ...
        Message = "Found more than 1 {Header} header. Skipping it."
    )]
    public static partial void HeaderOccursMoreThanOnce(this ILogger logger, string header);

    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "{Header} header has invalid value {Value}. Skipping it."
    )]
    public static partial void InvalidHeader(this ILogger logger, string header, string value);

    [LoggerMessage(
        Level = LogLevel.Error,
        Message = "Invalid header line: {Line} Expected: {{key}}:{{value}}"
    )]
    public static partial void InvalidHeaderLine(this ILogger logger, string line);

    [LoggerMessage(
        Level = LogLevel.Error,
        Message = "Invalid posting status {Status} in line: {Line}"
    )]
    public static partial void InvalidPostingStatus(
        this ILogger logger,
        string status,
        string line
    );
    ...
    [LoggerMessage(
        Level = LogLevel.Error,
        Message = "Invalid newsgroup origin line: {Line} Expected: {{group}} {{timestamp}} {{createdby}}"
    )]
    public static partial void InvalidGroupOriginLine(this ILogger logger, string line);

    [LoggerMessage(
        Level = LogLevel.Error,
        Message = "Invalid newsgroup information line: {Line} Expected: {{group}} {{high}} {{low}} {{status}}"
    )]
    ...
    public static partial void InvalidGroupExtendedInformationLine(
        this ILogger logger,
        string line
    );
    ...
    [LoggerMessage(
        ...
        Message = "Creating new NNTP client ({CurrentPoolSize}/{MaxPoolSize})"
    )]
    public static partial void CreatingNewNntpClient(
        this ILogger logger,
        int currentPoolSize,
        int maxPoolSize
    );
    ...
    [LoggerMessage(
        Level = LogLevel.Debug,
        Message = "Disposing idle NNTP client ({CurrentPoolSize}/{MaxPoolSize})"
    )]
    public static partial void DisposingIdleNntpClient(
        this ILogger logger,
        int currentPoolSize,
        int maxPoolSize
    );
    ...
    [LoggerMessage(
        Level = LogLevel.Error,
        Message = "Disposing NNTP client left in an errored state after a previous operation ({CurrentPoolSize}/{MaxPoolSize})"
    )]
    public static partial void DisposingErroredNntpClient(
        this ILogger logger,
        int currentPoolSize,
        int maxPoolSize
    );
